...
struct KmpTest :testing::Test, testing::WithParamInterface<testState> {
    void SetUp() {
      std::cout << GetParam();
    }
};

TEST_P(KmpTest, searchPatternInText) {
  auto state = GetParam();
  ASSERT_EQ(state.results, SearchSubstring(state.string, state.pattern));
}


INSTANTIATE_TEST_CASE_P(Default, KmpTest,
  testing::Values(
  testState{"a", "a", {0}},
